...
class BoardOutException(BoardException):
    def __str__(self):
        return f'Клетка {int_to_char(self.x)}{add_one(self.y)} вне доски'
...
class TooLargeBoardException(Exception):
    def __str__(self):
        return f'Размер доски должен быть не больше 26'


class TooLargeShipException(Exception):
    def __str__(self):
        return f'Длина коробля должена быть не больше 5'
...
class WrongCharException(Exception):
    def __str__(self):
        return f'Введите строчную букву латиницы'
...
class Board:
    ...
    def __str__(self):
        parts = [' ' * (len(str(self.size)) + 1),
                ' '.join(map(int_to_char, range(0, self.size))), '\n',
                ' ' * len(str(self.size)), '╔', '═' * self.size * 2, '╗', '\n']

        for y in range(self.size):
            parts.append(str(add_one(y)))
            parts.append(' ' * (len(str(add_one(self.size))) - len(str(add_one(y)))))
            parts.append('║')
        ...
        parts += [' ' * len(str(self.size)), '╚', '═' * self.size * 2, '╝', '\n']
        return ''.join(parts)
...
class Ship:
    def __init__(self, length, orientation, x, y):
        ...
        self.lives = length
        self.space_cells = []
        self.names = ['Катер', 'Эсминец', 'Крейсер', 'Линкор', 'Авианосец']
...
class Player:
    def __init__(self, game, enemy, colorCode):
        self.enemy = enemy
        self.game = game
    ...
    def ask(self):
        ...
        print(self.colorCode.format(self.enemy))
        print('\033[0m')
...
class AI(Player):
    ...
    def ask(self):
        ...
        print(f'Ход компьютера: {int_to_char(shot_cell.x)}{add_one(shot_cell.y)}')
        shot_cell.fire()
...
class User(Player):
    def ask(self):
        while True:
            input_string = input("Ваш ход: ").replace(' ', '')

            x, y = input_string[0:1], input_string[1:]

            if not x.isalpha():
                print("Первая координата должна быть буквой! ")
                continue

            if not y.isdigit():
                print("Вторая координата должна быть числом! ")
                continue

            try:
                x, y = char_to_int(x), subtract_one(int(y))
            except WrongCharException as e:
                print(e)
                continue
...
class Game:
    def __init__(self):
        ...
        self.user = User(self, self.computer_board, '\033[34m {}')
        self.computer = AI(self, self.user_board, '\033[31m {}')
    ...
    def greet():
        print("Дороу ")
        print("формат ввода: <столбец><строка>")
        print()

    def loop(self):
        print("Доска пользователя:")
        print('\033[0m')
        print('\033[37m {}'.format(self.user_board))
        print('\033[0m')
        print()
